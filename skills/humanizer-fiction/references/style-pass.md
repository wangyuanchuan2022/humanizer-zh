...
**v7 跨篇模具黑名单（实测已暴露形态；生成时禁用，自检时猎查）**——与本表单篇猎查不同，模具是**跨篇复用**签名（同一批生成的多篇共用同一句式构造；实测被评审以「跨不同作者重复出现」点名定罪）：

| 模具形态（猎查正则） | 实测出处（本项目 eval-v5） |
|---|---|
| `wearing a .{0,20}(costume|disguise|lanyard)` | 生成臂 3 篇命中、真人臂 0（语料层复核） |
| `(caveat|verdict|stance), so you can \w+ me` | 评审点名 12 处 |
| `If I compress .{0,30} into` 收尾盘点 | 评审点名 4 处 |

纪律：同批生成多篇时维护「已用句式」台账（draw sheet 的 used_before 字段）——**任何句式模具不得跨篇复用**；新发现的模具形态回流登记本表（批级自检②的输出）。写自检时对以上正则跑全文匹配，命中即打回改写（不是删句完事——换成非模板的自然表达）。
...
**v7 英文分布带（E 级修订，附溯源）。** sepia 原决策是「句长平均值随模型世代反转，不能写目标值」——本文件此前因此**只写方向不写数字**。v7 修订：本项目六文体三臂实测发现中英落差 60-100pp（中文支线有 `zh-hans.md` 数字带、英文侧无等价物是原因之一），故英文侧引入**分布带**——给带、带内抽签（draw sheet D.3）、不锁点值；这不是回到「目标值」（带不是值），而是给「方向」配上可执行的落点。带的数字来源：本项目真人臂只读统计（bands-from-human-corpus.md，en 三文体：篇长/句长 SD/段长 SD/第一人称/千字/数字/千字——**这些是本语料的实测带，不是普适常数**，跨语料须重测）。原「均值非信号」的判定保持不变；新增的是「**离散度**带可执行」。

**v7 元叙事提示语黑名单（英文侧；中文侧见 zh-hans.md）。** 实测（E）：生成臂 48-72% 篇目含元话语-让步丛（真人 12-33%）。生成时禁用、自检时猎查：

- "this is the part of the story where…" / "That's the part nobody…"
- "Let me be honest / fair / clear / up front" / "I want to be honest with you"
- "Here's the thing / Here's what…" 开段
- "Nobody tells you / Nobody warns you…"（作为主题句）
- "If I'm being honest / to be honest with you" 高频出现（>1 次/篇即命中）
...
| 参数 | en-essay（个人随笔） | en-tech（技术文章） | en-fiction（小说） |
|---|---|---|---|
| 篇长带（词） | [453, 1524]（SD 317——**方差大是本文体特征**） | [551, 929]（SD 92——技术文篇幅刚性） | [267, 1332]（SD 367） |
| 句长 SD 带 | [5.8, 16.2] | [7.8, 23.1] | [5.6, 18.3] |
| 段长 SD 带 | [11.4, 54.4] | [17.6, 61.4] | [9.7, 74.5] |
| 第一人称/千字 | [16.3, 113.2]（随笔天然第一人称密） | [2.9, 74.3]（技术文跨度大——从团队博客到个人吐槽） | [0, 113.1]（第三人称小说可为 0） |
| 缩略率 | 高（I'm/don't/can't 常态） | 中（正式技术文低、个人博客高） | 按对话/叙述分层 |
| 词汇平实度 | 中（允许文气，但每段发力即指纹） | 高（技术精确性优先） | 按视角人物语气 |

（带数值来源：bands-from-human-corpus.md；缩略率与平实度为定性档——sepia 未提供数字，本项目未测，标注定性。）
