<!-- v5.0 编译自 sepia references/rubric.md（MIT）+ humanizer v4.6 资产。
     相对 sepia 原版报告模板的适配：Voice fit 行省略（v5 无 voices 机制）。
     v6 迁移注记：Group F（中国程式文书指纹）已迁至 humanizer-zh-write 包（zh-rubric-f.md）——
     仅当诊断对象是中文个人文书且该包已加载时，按其指针表在报告追加 Group F 行；纯叙事诊断略过。 -->
...
（v5 相对 sepia 原模板：Voice fit 行省略——v5 无 voices 机制。v6 迁移注记：Group F 行仅当诊断对象是中文个人文书且 humanizer-zh-write 包已加载时追加——按其 zh-rubric-f.md 指针表判定；纯叙事诊断略过该行。其余各行逐一保留。）
...
Group D: 标记观测 … （具名互文出现——「…」）
Group E: 观测信号 … （…）
Group F: 指纹观测 — <引文证据>；n/a …   （仅当诊断对象是中文个人文书且 humanizer-zh-write 包已加载：按其 zh-rubric-f.md 指针表判定；否则略过本行）
