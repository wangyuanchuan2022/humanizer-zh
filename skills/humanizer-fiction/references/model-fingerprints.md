...
### 训练读者四判据（E；v7b 盲测 2026-09，两轮 30/30 全中）

实测条件：已训练读者（两轮全中）对 v7 生成臂逐篇判定 + 事后机制归因——**priming 下成立**（第二轮带第一轮指纹假设进场），朴素读者命中率待测（盲测装置改外部读者，v7.1 轮补测）。四判据与 v7.1 机制对照（Phase A 32 篇人臂语义标注佐证，2026-09-21）：

| 判据（读者归因） | v7 机制根因 | Phase A 实测佐证 | v7.1 对策 |
|---|---|---|---|
| ① 细节「为真实性服务」——工具化人味 | D.4b 配额使非指向细节每篇在场且要求「实质」——配额本身赋予功能 | doc/s2 真人臂 non_goal 实测全零/近零（8/8 与 10/12）——真人叙事细节皆承重 | 0 加权文体分化（D.4b）+ 链长参数（D.10，孤儿细节 57%） |
| ② 信息密度异常均匀（数量随机、位置不随机） | 抽签只随机化数量，位置由写作会话默认均匀铺开 | 非目标细节偏前（7/3/2）、坏痕偏后（3/6/9）——两类噪声互补的位置结构 | noise_position（D.10）+ moves 交集 ≤2（D.10 约束 1） |
| ③ 漂亮但不必要的句子总在发力位 | coinage/aphorism 配额只管数量，无位置约束 | 人臂最有力句 end 位仅 52%（middle 37% / start 11%） | power_position（D.10 + style-pass v7.1 节） |
| ④ 真人「坏得无意义」（无后见之明整理的痕迹） | 全部瑕疵手法（自嘲/记忆模糊/错字）都有可辨识的叙事/防御功能——连不完美都是设计出来的 | 坏痕在场 47%（doc 最高 75%）；形态实测=冗余重述/语法杂糅/词语粘连/事实性自相矛盾/成语误用，**预设四形态零实证** | meaningless_badness 族（D.10，不迷人的坏） |

**方法论注记**：该轮同时证实——逐篇独立会话**不改变**可识别性（机制层手法池聚合签名是主导信号，会话声音不是变量）；判据④的时间线混乱实例（zh-doc-010「周一(3月8号)」vs「3月9号(周一)」）与语料标注独立发现同篇同特征，判据获语料交叉验证。
